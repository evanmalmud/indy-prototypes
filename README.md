...
Read these in order. The audit measures, the comparison argues, the backlog acts.

| Document | What it is |
| --- | --- |
| [`AUDIT.md`](AUDIT.md) | **Measured, not asserted.** Generated by `npm run audit` from exhaustive search over the pure sims. Every level's shortest route, which interactions actually fire, and the headline **forced/declared** number — each declared interaction is deleted from the shared table and the level re-solved, so a designed insight the puzzle does not force is recorded as a suggestion. No number in it was typed by hand. |
| [`COMPARISON.md`](COMPARISON.md) | **The decision document.** Grades the three pillars against the audit rather than against their own memos, scores them on eight criteria, answers whether the aha comes from the interactions or from the pillar wrapped around them, evaluates ability-gating and two other hybrids, and commits to a recommendation. |
| [`ITERATION-BACKLOG.md`](ITERATION-BACKLOG.md) | **What to do next.** Three concrete changes per pillar, ordered by value per hour, each naming the files it touches and the audit number that motivates it. |

The short version of the comparison: the flagship `WHIP+BOULDER` is forced in
every room it appears in across all three pillars, while `SATCHEL+PRESSURE_PLATE`
is flagged unforced in two *different* pillars — so aha strength clusters by
interaction, not by pillar. Three of the six seeded ahas have never been forced in
any room in this project.
...
**Pillar B is partially built and should be read as unfinished** — six rooms and a
1,306-line sim exist at `src/pillar-b/`, and the audit searched all six. But its
page is still a placeholder (`src/pillar-b/main.ts`), so it has no undo despite
being the only pillar with a real death state; its capstone `b6` has no authored
route and no win assertion; its tests are a scratchpad rather than a suite; it has
no `NOTES.md`; and several comments cite a `src/pillar-b/sim.test.ts` that does not
exist. Its audit numbers are therefore confounded with incompleteness — see
[`ITERATION-BACKLOG.md`](ITERATION-BACKLOG.md) for what would settle it.
